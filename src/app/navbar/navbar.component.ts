...
ngOnInit(){
 if (this.productSharing.getAllProductsSnapshot().length > 0) {
    this.products = this.productSharing.getAllProductsSnapshot();
  } else {
    this.productSharing.getAllProducts().subscribe(products => {
...
searching() {
  ...
  this.serarchingArray = this.products.filter(p =>
